...
Ship::Ship()
{
	Position = Vector3D(12.0,0.0,0.0);
	Forward = Vector3D(-1.0,0.0,0.0);
	Right = Vector3D(0.0,0.0,-1.0);
	Up = Vector3D(0.0,1.0,0.0);

  mesh = ReadOffFile("ship_file.off");
  if (!mesh) exit(-1);

}

void Ship::TranslateForward(GLfloat dist)
{
	Position = Position + (Forward * dist);
}

void Ship::TranslateRight(GLfloat dist)
{
	Position = Position + (Right * dist);
}

void Ship::TranslateUp(GLfloat dist)
{
